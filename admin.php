        ...
        <div id="page-wrapper" >
			...
			<div class="panel panel-back noti-box">
              ...
              $result=$db->query('select count(*) as num from person where isadmin=0');
                ...
                <div class="text-box" >
					...
					if($result)
					{  
				        $usernum=$result->fetch();
					    echo '<p class="main-text">'.$usernum['num'].'名</p>';	
					}
					else
			...
			<div class="panel panel-back noti-box">
                ...
                <div class="text-box" >
                     <?php
					   $result=null;
			  $result=$db->query("select count(*) as num from place");
			   if($result)
			   {
				    $placenum=$result->fetch();
                    echo '<p class="main-text">'.$placenum['num'].'个</p>';
			   }
               else 
				   echo '<p class="main-text">0个</p>';
                         ?>                
				<p class="text-muted">景点</p>
                </div>
             </div>
		     </div>
                    <div class="col-md-3 col-sm-6 col-xs-6">           
			<div class="panel panel-back noti-box">
                ...
                <div class="text-box" >
                      <?php
					   $result=null;
			  $result=$db->query("select count(*) as num from photo");
			   if($result)
			   {
				    $recordnum=$result->fetch();
                    echo '<p class="main-text">'.$recordnum['num'].'张</p>';
			   }
               else 
			 ...
			 <div class="panel panel-back noti-box">
                ...
                <div class="text-box" >
                     <?php
					   $result=null;
			  $result=$db->query("select count(*) as num from diary ");
			   if($result)
			   {
				    $recordnum=$result->fetch();
                    echo '<p class="main-text">'.$recordnum['num'].'篇</p>';
			   }
               else 
